...
public:
  ...
  void constituentsOf(const EcalTrigTowerConstituentsMap& ttMap, 
		      const EcalTrigTowerDetId& id, 
		      std::vector<DetId> xtals);
...
private:
  ...
  // track quality and number of valid hits and layers
  int trkQuality1, trkQuality2, trkNVhits1, trkNVhits2;
  int Nlayers1, Nlayers2, NpxlHits1, NpxlHits2, NoutLayers1, NoutLayers2;
  int 	idmax1, idmax2;
...
SinglePionEfficiencyNew::SinglePionEfficiencyNew(const edm::ParameterSet& iConfig) : conf_(iConfig)
  //ebSrFlags_(iConfig.getParameter<edm::InputTag>("EbSrFlagCollection"), false,collNotFoundWarn_)
{
...
SinglePionEfficiencyNew::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   idmax1 = -1; idmax2 = -1;
   //
   Nlayers1 = 0; NpxlHits1 = 0; NoutLayers1 = 0;
   ...
   for(CaloTowerCollection::const_iterator towerItr = towers->begin(); 
     ...
     cout <<" tower e = " << towerE 
	  <<" eta = " << towerEta
	  <<" phi = " << towerPhi
	  <<" emEnergy() = " << towerItr->emEnergy()
	  <<" hadEnergy() = " << towerItr->hadEnergy()
	  <<" outerEnergy() = " << towerItr->outerEnergy()
	  <<" ieta() = " << towerItr->ieta() 
	  <<" iphi() = " << towerItr->iphi() << endl; 
   }
   */
   ...
   for(size_t j = 0; j < theSimTracks.size(); j++){
	...
	track != tracks->end(); track++) {
      ...
      //      int NpxlHits   = track->hitPattern().numberOfValidPixelHits();
      int NpxlHits   = track->hitPattern().pixelLayersWithMeasurement();
      int NoutLayers = track->hitPattern().stripTOBLayersWithMeasurement() +
      ...
      int nOuterHits     = hitp.stripTOBLayersWithMeasurement()+hitp.stripTECLayersWithMeasurement() ;
            >= 4
      http://cmslxr.fnal.gov/lxr/source/DataFormats/TrackReco/interface/HitPattern.h?v=CMSSW_2_2_13

       */

      /*
      const reco::HitPattern& hitPattern = track.hitPattern();
      const size_t nHits = hitPattern.numberOfHits();
      uint16_t nPixelHits = 0;
      for (size_t i = 0; i < nHits; ++i) {
	const uint32_t pattern = hitPattern.getHitPattern(i);
	if ( hitPattern.pixelHitFilter(pattern) && hitPattern.validHitFilter(pattern) ) nPixelHits++;
      }
      */
	...
	for(CaloTowerCollection::const_iterator towerItr = towers->begin(); 
	  ...
	  double towerEta = towerItr->eta();
	  double towerPhi = towerItr->phi();
	  double DR = deltaR(point.eta(), point.phi(),towerEta,towerPhi);
	  if(DR < 0.2 && towerE != 0.) {
	...
	*/
	...
	Nlayers1 = Nlayers; 
	NpxlHits1 = NpxlHits;
	NoutLayers1 = NoutLayers;
	eCaloTowers1 = eCaloTowers;
      }
	...
	Nlayers2 = Nlayers; 
	NpxlHits2 = NpxlHits;
	NoutLayers2 = NoutLayers;
	eCaloTowers2 = eCaloTowers;
      }
      t++;
    }
   ...
   // fill tree
    ...
    //
    /*
    std::vector<edm::Provenance const*> prov;
    iEvent.getAllProvenance(prov);
    for(unsigned int i=0; i< prov.size(); ++i){
      if(prov[i]->moduleName() == string("EcalSelectiveReadoutProducer")){
	cout << "--------------------------------------------------------------------->\n";
	cout << "Module label: " << prov[i]->moduleLabel() << "\n"
	     << "Product instance name: " << prov[i]->productInstanceName() << "\n"
	     << "Module Name: "  << prov[i]->moduleName() << "\n"
	...
	iEvent.getByLabel(prov[i]->moduleLabel(), prov[i]->productInstanceName(), hEbSrFlags);
	if(hEbSrFlags.isValid()){
	  cout << "EB Collection size: " << hEbSrFlags->size() << " flags.\n\n";
	}
	
	edm::Handle<EESrFlagCollection> hEeSrFlags;
	iEvent.getByLabel(prov[i]->moduleLabel(), prov[i]->productInstanceName(), hEeSrFlags);
	if(hEeSrFlags.isValid()){
	  cout << "EE Collection size: " << hEeSrFlags->size() << " flags.\n\n";
	}
	cout << "----------------------------------------------------------------------\n\n";
      }
    }
    */
    ...
    // SR flags
    /*
    edm::Handle<EBSrFlagCollection> hEbSrFlags_;
    iEvent.getByLabel("simEcalDigis", "ebSrFlags", hEbSrFlags_);

    edm::Handle<EESrFlagCollection> hEeSrFlags_;
    iEvent.getByLabel("simEcalDigis", "eeSrFlags", hEeSrFlags_);

    edm::Handle<EcalTrigPrimDigiCollection> ecalDigi;
    iEvent.getByLabel("simEcalTriggerPrimitiveDigis",ecalDigi);
    //  iEvent.getByLabel("simHcalTriggerPrimitiveDigis",hcalDigi);
	...
	//	  srf = itSrfee->value(); //*itSrf instead of itSrf->value() should also work
	  ...
	  cout <<" ieta = " << ecalTower->id().ieta()
	       <<" iphi = " << ecalTower->id().iphi()
	       <<" Et " << ecalTower->compressedEt() 
	       <<" towerWasReadInFullReadoutMode = " << towerWasReadInFullReadoutMode << endl;
	  // it is in ADC. 1 ADC = 468.75 MeV
	  std::vector<DetId> xtals = ttMap.constituentsOf(ecalTower->id());
	  cout <<" crystal matrix size = " << xtals.size() << endl;
	  double sume = 0.; 
	  double energy = 0.;
	  for(int i = 0; i < xtals.size(); ++i) {
	    EBDetId crystal(xtals[i]);
	    EcalRecHitCollection::const_iterator hit;
	    hit = barrelRecHitsHandle->find(xtals[i]);
	    if (hit != barrelRecHitsHandle->end()) 
	      {
		energy = hit->energy();
	      }
	    sume = sume + energy;
...
SinglePionEfficiencyNew::beginJob(const edm::EventSetup& iSetup)
{
  ...
  //
  t1->Branch("Nlayers1",&Nlayers1,"Nlayers1/I");
  t1->Branch("NpxlHits1",&NpxlHits1,"NpxlHits1/I");
  t1->Branch("NoutLayers1",&NoutLayers1,"NoutLayers1/I");
  t1->Branch("Nlayers2",&Nlayers2,"Nlayers2/I");
  t1->Branch("NpxlHits2",&NpxlHits2,"NpxlHits2/I");
...
void SinglePionEfficiencyNew::constituentsOf(const EcalTrigTowerConstituentsMap& ttMap,
					     const EcalTrigTowerDetId& id, 
					     std::vector<DetId> xtals)
					     //					     std::vector<EcalTriggerElectronicsId>& xtals)
{
  cout <<"          before " << endl;
  std::vector<DetId> xtals_ = ttMap.constituentsOf(id);
  cout <<"           after " <<" size = " << xtals_.size() << endl;
  //  for(int i = 0; i < xtals_.size(); ++i) {
  //    xtals[i] = xtals_[i];
  //  }
  xtals = xtals_;
  cout <<"            after " <<" size = " << xtals.size() << endl;
}
