...
class DiMuAnalysis_Taus : public edm::EDAnalyzer {
public:
  explicit DiMuAnalysis_Taus(const edm::ParameterSet&);
  ~DiMuAnalysis_Taus();
...
void 
//DiMuAnalysis_Taus::beginJob(const edm::EventSetup&)
DiMuAnalysis_Taus::beginJob()
{
  using namespace edm;
...
//
DiMuAnalysis_Taus::DiMuAnalysis_Taus(const edm::ParameterSet& iConfig)
...
DiMuAnalysis_Taus::~DiMuAnalysis_Taus()
{
...
void
DiMuAnalysis_Taus::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
  ...
  //caloRecoTauDiscriminationByECALIsolation
   ...
   iEvent.getByLabel("caloRecoTauDiscriminationByLeadingTrackFinding",
		     theCaloTauDiscriminatorByLeadingTrackFinding);
   //
   Handle<CaloTauDiscriminator> theCaloTauDiscriminatorByLeadingTrackPtCut;
   iEvent.getByLabel("caloRecoTauDiscriminationByLeadingTrackPtCut",
		     theCaloTauDiscriminatorByLeadingTrackPtCut);
   //
   Handle<CaloTauDiscriminator> theCaloTauDiscriminatorByIsolation;
   iEvent.getByLabel("caloRecoTauDiscriminationByIsolation",
		     theCaloTauDiscriminatorByIsolation);
   //
   Handle<CaloTauDiscriminator> theCaloTauDiscriminatorAgainstElectron;
   iEvent.getByLabel("caloRecoTauDiscriminationAgainstElectron",
   ...
   for(iTau = caloTaus.begin(); iTau != caloTaus.end(); iTau++) {
     ...
     cout <<"      pT = " << iTau->pt() 
	  <<" pT raw = " << RawCaloJet->pt() 
	  <<" eta = " << fabs(iTau->eta()) 
	  <<" eta = " << fabs(RawCaloJet->eta()) << endl;
     
     DiscriminatorByLeadingTrackFinding = 
       (*theCaloTauDiscriminatorByLeadingTrackFinding)[theCaloTauRef];

     DiscriminatorByLeadingTrackPtCut   = 
       (*theCaloTauDiscriminatorByLeadingTrackPtCut)[theCaloTauRef];

     DiscriminatorByIsolation           = 
       (*theCaloTauDiscriminatorByIsolation)[theCaloTauRef];
...
DEFINE_FWK_MODULE(DiMuAnalysis_Taus);
